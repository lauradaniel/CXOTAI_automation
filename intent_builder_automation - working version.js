...
async function executeMoveMerge(targetLi, changePath) {
  ...
  async function run() {
    ...
    // Helper to safely extract column values ignoring hidden characters/spaces from Excel
      ...
      // 1. Normalize the action text (lowercase, remove extra spaces)
      const cleanAction = action.trim().toLowerCase();
      
      // 2. Define the exact list of acceptable actions (leaving 'remove' as a safe fallback)
      const validActions = ['rename', 'move', 'merge', 'deactivate', 'remove'];

      // 3. Skip immediately if the action is empty or not in our approved list
      if (!cleanAction || !validActions.includes(cleanAction)) {
        log(`Row ${rowNum}: Action "${action || 'EMPTY'}" ignored — skipping "${label}"`, 'warn');
        skipped++; continue;
      }
